...
from sklearn.metrics import (accuracy_score, confusion_matrix, f1_score, precision_score, recall_score, RocCurveDisplay, ConfusionMatrixDisplay)
...
class FakeNewsPrediction(object):
    ...
    ***Fake News Prediction***
    ...
    def model_training(self):
        ...
        estimator = LogisticRegression(solver="liblinear", random_state=self.randstate)
        genet = GAFeatureSelectionCV( estimator, cv=5, verbose=0, scoring="accuracy", population_size=50, generations=50, crossover_probability=0.5, 
                                     mutation_probability=0.2, tournament_size=3, n_jobs=-1, keep_top_k=2)
        ...
        # AUC and ROC curve
        roc_disp = RocCurveDisplay.from_estimator(self.model, x_test, y_test)
        roc_disp.figure_.suptitle("%s: ROC curve comparison" % name)
        roc_disp.figure_.savefig(os.path.join(output_folder, "%s_ROC_Curve.png" % name), bbox_inches="tight")
        roc_disp.figure_.clf()
        ...
        plt.savefig(os.path.join(output_folder, "%s_cm_test_dataset.png" % name), bbox_inches="tight")
        plt.clf()
    ...
    def model_prediction(self):
        ...
        if hasattr(self.model, "predict_proba"):
            ...
            # fallback: use decision_function scores
            preds = self.model.decision_function(x_data)
    ...
    def main(self, caller_json):
        ...
        if status == 1:
            description = "Model run finished successfully at %s (runtime %.2f secs)" % (time.strftime("%Y-%m-%d %H:%M:%S"), (time.time() - self.model_start))
        else:
